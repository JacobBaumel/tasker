...
namespace tasker
{
    ...
    sql::ResultSet* show_available_databases(sql::Connection* connection)
    {
        ...
        sql::ResultSet* set = stmt->executeQuery("show databases");
        delete stmt;
        return set;
    }
    ...
    {
        sql::ResultSet* set = show_available_databases(connection);
        while (set->next())
        {
            if (set->getString(1) == properties.db_name)
            {
                delete set;
                return false;
            }
        ...
        delete set;
        sql::Statement* stmt = connection->createStatement();
        stmt->executeUpdate("create database " + properties.db_name);

        try
        {
            connection->setSchema(properties.db_name);

            // Sets up metadata
            stmt->executeUpdate("create table MetaData (MetaKey varchar(64), MetaValue varchar(512))");
            stmt->executeUpdate("insert into MetaData (MetaKey, MetaValue) values ('org_name', '" + properties.org_name + "')");
            stmt->executeUpdate("insert into MetaData (MetaKey, MetaValue) values ('workspace_name', '" + properties.workspace_name + "')");
            stmt->executeUpdate("insert into MetaData (MetaKey, MetaValue) values ('description', '" + properties.description + "')");

            stmt->executeUpdate("create table TaskComments (SuperTaskName varchar(128) not null, taskId int not null, comment varchar(512) not null)");
            stmt->executeUpdate("create table TaskPeople (SuperTaskName varchar(128) not null, taskId int not null, personId int not null)");

            // sets up people table
            stmt->executeUpdate("create table People (id int not null AUTO_INCREMENT, name varchar(128), role varchar(256), primary key(id))");

            // sets up status table
            stmt->executeUpdate("create table Stati (id int not null AUTO_INCREMENT, name varchar(64), description varchar(256), colorR tinyint unsigned, colorG tinyint unsigned, colorB tinyint unsigned, primary key(id))");
        }
        catch (sql::SQLException& e)
        {
            std::cerr << "Error attempting to initialize workspace!" << std::endl;
    ...
    bool create_status(sql::Connection* connection, const std::string& name, const Color& color, const std::string& description) {
        sql::PreparedStatement* stmt = connection->prepareStatement("insert into Stati (name, description, colorR, colorG, colorB) values (?, ?, ?, ?, ?)");
        try {
            stmt->setString(1, name);
            stmt->setString(2, description);
            stmt->setInt(3, color.red);
            stmt->setInt(4, color.green);
            stmt->setInt(5, color.blue);
            stmt->executeUpdate();
            delete stmt;
            return true;
        } catch(sql::SQLException& e) {
    ...
    bool modify_status_name(sql::Connection* connection, int id, std::string& name) {
        sql::PreparedStatement* stmt = connection->prepareStatement("update Stati set name=? where id=?");
    ...
    bool modify_status_color(sql::Connection* connection, int id, const Color& color) {
        std::string vals ((color.red < 0 ? "" : "colorR = " + std::to_string(color.red)));
        vals += color.green < 0 ? "" : (vals.empty() ? "" : ", ") + (std::string) "colorG = " + std::to_string(color.green);
        vals += color.blue < 0 ? "" : (vals.empty() ? "" : ", ") + (std::string) "colorB = " + std::to_string(color.blue);
        if(vals.empty()) return false;

        sql::PreparedStatement* stmt = connection->prepareStatement("update Stati set " + vals + " where id=?");
        try {
            stmt->setInt(1, id);
            stmt->executeUpdate();
            delete stmt;
...
std::unordered_map<std::string,dataType>  RSJresource::as< std::unordered_map<std::string,dataType> > 
    ...
    bool modify_status_description(sql::Connection* connection, int id, std::string& description) {
        sql::PreparedStatement* stmt = connection->prepareStatement("update Stati set description=? where id=?");
    ...
    bool delete_status(sql::Connection* connection, int id) {
        sql::PreparedStatement* stmt = connection->prepareStatement("delete from Stati where id=?");
    ...
    // [Section] Management of People table
    bool create_person(sql::Connection* connection, const std::string& name, const std::string& role) {
        sql::PreparedStatement* stmt = connection->prepareStatement("insert into People(name, role) values (?, ?)");
    ...
    bool modify_person_name(sql::Connection* connection, int id, const std::string& name) {
        sql::PreparedStatement* stmt = connection->prepareStatement("update People set name=? where id=?");
    ...
    bool modify_person_role(sql::Connection* connection, int id, const std::string& role) {
        sql::PreparedStatement* stmt = connection->prepareStatement("update People set role=? where id=?");
    ...
    bool delete_person(sql::Connection* connection, int id) {
        sql::PreparedStatement* stmt = connection->prepareStatement("delete from People where id=?");
    ...
    // [SECTION] Supertask modifiers
    bool create_supertask(sql::Connection* connection, const std::string& name, const Color& color) {
        sql::Statement* stmt = connection->createStatement();

        try {
            //stmt->setString(1, name);
            stmt->executeUpdate("create table " + name + " (id int not null AUTO_INCREMENT, name varchar(64) not null, people JSON, complete_date DATE, status_id int not null, comments JSON, appearence_order int not null, primary key (id))");
            delete stmt;
            stmt = connection->createStatement();
            stmt->executeUpdate("insert into MetaData (MetaKey, MetaValue) values ('" + name + "_color', '" + std::to_string(color.red) 
                + "_" + std::to_string(color.green) + "_" + std::to_string(color.blue) + "')");
            delete stmt;
            return true;
        } catch(sql::SQLException& e) {
            std::cerr << e.what() << std::endl;
